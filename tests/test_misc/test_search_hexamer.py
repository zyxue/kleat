...
from search_hexamer import (
    plus_search, minus_search, fetch_seq, gen_coords, search)


REF_FA = '/projects/btl/zxue/tasrkleat-TCGA-results/utrtargets/reference/Homo_sapiens.GRCh37.75.dna_sm.primary_assembly.fa'
...
class TestSearch(unittest.TestCase):
    ...
    def test_chr12_DRAM_plus_strand(self):
        ...
        self.assertEqual(
            fetch_seq(self.refseq, '12', 102316872, 102316872 + 6), 'aataaa')
        self.assertEqual(
            search(self.refseq, chrom, clv, '+', 50), ('AATAAA', 1, 102316872))
    ...
    def test_chr19_AKT2_minus_strand(self):
        ...
        # confirm the corresponding seq in hg19
        self.assertEqual(
            fetch_seq(self.refseq, chrom, 40737011 + 1 - 6, 40737011 + 1), 'TTTATT')
        self.assertEqual(
            search(self.refseq, chrom, clv, '-', 50), ('AATAAA', 1, 40737011))


class TestPysamFetch(unittest.TestCase):
    def setUp(self):
        self.refseq = pysam.FastaFile('./mock_seq.fa')
